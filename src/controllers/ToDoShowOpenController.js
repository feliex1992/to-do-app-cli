...
class ToDoShowOpenController extends ControllerBase {
  async list() {
    let listMenu = [];
    await qoa.clearScreen();
    console.log(`Wellcome ${userStore.data.email} [To Do App CLI]`);

    const showTodos = todosStore.data.filter((todo) => todo.statDone === false);
    listMenu = showTodos.map((t, i) => ({_id: t._id, content: `[${i + 1}]: ${moment(t.createdAt).format('YYYY-MM-DD HH:mm:ss')} | ${t.content} | ${t.tags}`}));
...
module.exports = new ToDoShowOpenController;